...
class Hand
	def initialize(cards)
		...
		def is_a_straight?
			...
			[5,4,3,2,9]
			card_values.sort!
			card_values.each_cons(2).all? {|first,scond| second = first + 1}
		end
...
def validate_draw_input(input)
	return puts 'sorry ,I didn\'t understnd that.'unless input.is_a?(Integer)
	return puts 'you can\'t discard the same card twice!'if @crads_to_discard.include?(input)
